...
EventMirror.prototype = {
  ...
  _addListener: function(data, socket) {
    ...
    debug('asking to mirror: ' + data.events.join(', '), '; id is', id);
    ...
    socket.once('close', function onMirrorSocketClose() {
      debug('socket', id, 'has closed');
      self._removeListener(ack, socket);
    });
  ...
  _removeListener: function(data, socket) {
    var id = data.id,
        events = data.events,
        listenerEvents,
        self = this,
  ...
  _captureEmit: function(event, data) {
    ...
    if (event in this.capturedEvents) {
      ...
      for (i; i < list.length; i++) {
        ...
        if (mirror) {
          debug('mirroring', event, 'to', list[i]);
  ...
  _mirrorEvent: function(mirror, event, data) {
    try {
      mirror.socket.send(
        ...
        function(error) {
          if (error) {
            debug('Error trying to send to websocket proxy: ', error);
          }
        }
      );
    } catch (e) {
